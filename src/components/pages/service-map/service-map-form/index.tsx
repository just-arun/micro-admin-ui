      ...
      {_props.data ?
        <form onSubmit={(e) => {
          ...
          {_props.type == "update" ? <div className="flex items-center py-2 ">
            <span className="pr-4">Auth: </span>
            <input type="checkbox" id="authV" value={"id"} checked={_props.data.auth} onChange={(e) => _onUpdate("auth", e.target.checked)} />
            {/* <Toggle required id="auth" value={_props.data.auth} onChange={(e) => _onUpdate("auth", e)} /> */}
          </div> : ""}
          <div>
